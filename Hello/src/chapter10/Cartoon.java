...
public class Cartoon extends JPanel
{
	...
	protected void paintComponent(Graphics g)
	{
		...
		g2D.translate(5,3.3);                                         //Къде се намира слънцето
		paint_sun(g2D);
	...
	void paint_windmill(Graphics2D g2D)
	{
		...
		g2D.translate(0,3);                                              //местоположение на витлата
		g2D.rotate(-number);
		g2D.setColor(new Color(8, 43, 226 ));                            //цвят на витлата
		for (int i = 1; i <=6; i++)
		{
	...
	void paint_cars_plane(Graphics2D g2D)
	{
		...
		g2D.translate(16.5,-0.1);
		g2D.scale(0.8,0.8);
		paint_wheel(g2D);
		g2D.setTransform(transform);
		g2D.translate(13.5,-0.1);
		g2D.scale(0.8,0.8);
		paint_wheel(g2D);
		g2D.setTransform(transform);
		g2D.setColor(Color.YELLOW);
		g2D.fill(new Rectangle2D.Double(13.5,1,2,1.5) );
		g2D.fill(new Rectangle2D.Double(12.5,0,5,1.5) );
		g2D.setColor(Color.WHITE);
		g2D.fill(new Rectangle2D.Double(-9.7,1,0.8,1) );
		g2D.setColor(new Color(0,0,128));
		...
		g2D.fill(new Ellipse2D.Double(-5.5,12,4,1) );
		g2D.fill(new Rectangle2D.Double(-4.4,11.8,0.25,0.3) );                   //Дирижабъл
		g2D.fill(new Rectangle2D.Double(-5.3,12.1,0.20,0.9));
	}
